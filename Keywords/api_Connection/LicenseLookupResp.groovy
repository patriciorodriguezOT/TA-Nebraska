	...
	def calculateExpirationDate (ResponseObject responseLincenseLookup, String issueDate) {
		...
		if (currentYear % 2 == 0){
			if (licenseExpirationType == 'Expires in Odd Years') {
				...
				// If current year is Even and it Expires in Even Years, plus 2 year
				expiredYear = currentYear + 2
			}
		}else{
			if (licenseExpirationType == 'Expires in Odd Years') {
				// If current year is Odd and it Expires in Odd Years, plus 2 year
				expiredYear = currentYear + 2
			} else {
				expiredYear = currentYear + 1
